...
import { getLatestBlogs } from "@/sanity/lib";
...
const LatestBlog = async () => {
  ...
  return (
    <div className="mb-10 md:mb-20">
      <Title className="mt-1">Latest Blog</Title>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mt-5 rounded-lg">
        {blogs.map((blog) => (
          <div key={blog._id} className="hover:cursor-pointer rounded-lg overflow-hidden
            ...
            {blog.mainImage && (
              <Link href={`/blog/${blog?.slug?.current}`}>
                <Image
                  ...
                  width={500}
                  height={500}
                  className="object-contain w-full mac-h-80 "
                />
              </Link>
            )}
