...
const BlogLeft = async ({ slug }: { slug: string }) => {
  ...
  return (
    <div>
      <div className="border shadow-md shadow-shop_light_blue/20 p-5 rounded-md">
        ...
        <div className="space-y-2 mt-2">
          {categories?.map((blogcategories, index) => (
            ...
            >
              <p>{blogcategories?.blogcategories?.map((item)=>item?.title)}</p>
              <p className="text-darkColor font-semibold">{`(1)`}</p>
            </div>
          ))}
        </div>
      </div>
      <div className="border shadow-md shadow-shop_light_blue/30 p-5 rounded-md mt-10">
