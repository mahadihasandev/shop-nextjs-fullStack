          ...
          <Container>
           {groupedItems?.length?(
            ...
            <div className="flex items-center gap-2 py-5 shadow mb-2 pl-1 rounded-lg">
              <GiShoppingCart size={30} className="text-2xl text-text-darkColor/90"/>
              <h1 className="text-2xl font-sans font-bold text-darkColor/90">Shopping Cart</h1>
            </div>
            <div className="grid md:grid-cols-3 md:gap-8">
              <div className="md:col-span-2 lg:col-span-2 rounded-lg">
                <div className="border border-shop_light_blue/20 bg-shop_light_bg rounded-lg shadow-md 
                shadow-shop_light_blue/30">
                  {groupedItems?.map(({product})=>{
                    const itemCount=getItemCount(product?._id)
                    return(
                      <div key={product?._id} className="border-b shadow-md p-2.5 
                      last:border-b-0 flex items-center justify-between gap-5">
                        <div className="flex flex-1 items-start gap-2 h-36 md:h-44">
                          ...
                          <TooltipProvider>
                            <Tooltip>
                             <TooltipTrigger>
                               <AddToWishListButton className="h-10 w-10" product={product}/>
                             </TooltipTrigger>
                  ...
                  <Button variant="destructive" onClick={handleResetCart} className="m-5 font-semibold ml-[80%] lg:ml-[88%] md:ml-[85%] hover:scale-105">Reset</Button>
                </div>
              </div>
              <div>
                <div className="lg:col-span-1">
                  <div className="hidden md:inline-block w-full border border-shop_light_blue/20 bg-shop_light_bg p-6 rounded-lg shadow-md 
                  shadow-shop_light_blue/30">
                    <h2 className="text-xl font-semibold mb-4">Order Summary</h2>
                    <div className="space-y-4 border-t pt-2">
                      <div className="flex items-center justify-between font-sans">
                        <span>SubTotal:</span>
                        <PriceFormatter className="font-sans" amount={getSubtotalPrice()}/>
                      </div>
                      <div className="flex items-center justify-between border-b pb-2 font-sans">
                        <span>Discount:</span>
                        <PriceFormatter className="font-sans" amount={getTotalPrice()-getSubtotalPrice()}/>
                      </div>
                      
                       <div className="flex items-center justify-between font-semibold text-lg">
                        <span>Total:</span>
                        <PriceFormatter className="font-sans text-darkColor text-lg font-bold" amount={useStore?.getState().getTotalPrice()}/>
                      </div>
                      <Button size="lg" className="w-full rounded-full 
                      font-semibold tracking-wider 
                      hover:scale-105 mt-4" 
                      onClick={handleCheckout}>{loading ? "Loading..." : "Proceed to Checkout"}</Button>
                    </div>
                  </div>

                  
                  {
                  addresses&& <div className="border border-shop_light_blue/20 bg-shop_light_bg rounded-lg shadow-md 
                    ...
                    <Card className="bg-shop_light_bg shadow-none border-none">
                      <CardHeader>
                        ...
                        <CardContent>
                          <RadioGroup defaultValue={addresses?.find((addr)=>addr.default)?._id.toString()}>
                            {addresses?.map((address)=>(
                              ...
                              >
                               ...
                               <Label htmlFor={`address-${address?._id.toString()}`}
                          ...
                          <Button variant="outline" className="w-full hover:bg-shop_light_blue/20 font-sans mt-4">Add New Address</Button>
                        </CardContent>
                      </CardHeader>
                    </Card>
                  </div>
              ...
              {/* for smart phone */}
              <div className="md:hidden fixed bottom-0 w-full bg-shop_light_bg pt-2">
                <div className="border-shop_light_blue/20 bg-shop_light_bg p-6 rounded-lg shadow-md 
                  shadow-shop_light_blue/30 mx-4">
                  <h2>Order Summary</h2>
                </div>
              </div>
            </div>
